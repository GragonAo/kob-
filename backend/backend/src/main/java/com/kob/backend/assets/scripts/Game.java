...
public class Game extends Thread {
    ...
    class MoveItemInfo{
        ...
        private  Integer dir;
        public MoveItemInfo(Integer id,Integer d){playerId = id;dir = d;}
    }   //玩家移动信息格式

    private void sendMove(){
        JSONObject resp = new JSONObject();
        resp.put("event","game-move");
        List<MoveItemInfo>movelist = new ArrayList<>();
        for(Player player:players.values()){
            movelist.add(new MoveItemInfo(player.getId(),player.getNextStepOp()));
        }
    ...
    private void startGame(){
        ...
        for(Player player:players.values()){
            playerItems.add(new PlayerItem(player.getId(),player.getStartCell().r,player.getStartCell().c));
        }
